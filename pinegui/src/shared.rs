...
impl<T> AsRef<T> for SharedWeak<T> {
    fn as_ref(&self) -> &T {
        self
    }
}
...
impl<T: PartialEq> PartialEq for SharedWeak<T> {
    fn eq(&self, other: &Self) -> bool {
        **self == **other
    }
}

impl<T: Eq> Eq for SharedWeak<T> {}

impl<T> From<T> for SharedWeak<T> {
    fn from(value: T) -> Self {
        SharedWeak::new(value)
    }
}

impl<T: Clone> From<&T> for SharedWeak<T> {
    fn from(value: &T) -> Self {
        SharedWeak::new(value.clone())
    }
}

impl<T: Clone> From<&mut T> for SharedWeak<T> {
    fn from(value: &mut T) -> Self {
        SharedWeak::new(value.clone())
    }
}
