...
pub struct Ui<T: DrawTarget, C: FnOnce(Event) + Clone> {
    pub width: u32,
    pub height: u32,
    pub top_left: Point,

    pub render_list: Vec<Element<T, C>>,
    pub child_comp: Component<T, C>,
}

impl<T: DrawTarget, C: FnOnce(Event) + Clone> Ui<T, C> {
    pub fn new(width: u32, height: u32, top_left: Point, child_comp: Component<T, C>) -> Self {
        Self {
            width,
            height,
            top_left,
...
impl<T: DrawTarget, C: FnOnce(Event) + Clone> geometry::Dimensions for Ui<T, C> {
