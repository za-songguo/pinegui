...
pub struct State<V, T: DrawTarget, C: FnOnce(Event) + Clone> {
    value: V,
    ui: Ui<T, C>,
    /// A reference to the component that uses the value of this state.
    related_comps: Vec<Component<T, C>>,
}

impl<V, T: DrawTarget, C: FnOnce(Event) + Clone> State<V, T, C> {
    /// Create a new state with the given value.
    pub fn new(value: V, ui: Ui<T, C>) -> Self {
        Self {
            value,
            ui,
    ...
    pub fn value(&mut self, component: Component<T, C>) -> &V {
...
fn diff<T: DrawTarget, C: FnOnce(Event) + Clone>(
    a: Option<Vec<Element<T, C>>>,
    b: Option<Vec<Element<T, C>>>,
) -> Vec<Element<T, C>> {
